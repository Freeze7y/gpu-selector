...
在列表中可用 Ctrl / Shift 多选或 Ctrl+A 全选路径，再点击“清除所选路径”统一预览并删除所选图形设置，包括 Auto HDR 和窗口化优化；不删除应用文件。操作前备份、操作后读回验证，可在“备份与日志”恢复。
...
不再需要的备份可用 Ctrl / Shift 多选或 Ctrl+A 全选后“清除所选备份”，确认清单后移到 Windows 回收站。恢复备份仍需单选。暂时不可恢复不代表没有用；清除不会改变当前 GPU 设置。
