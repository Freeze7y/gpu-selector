...
- **清除旧路径**：单选或多选列表条目后，统一预览并清除所选路径的全部图形设置，包括 HDR 等参数；不删除程序文件，可从备份恢复。
...
- **备份清理**：单选或多选不需要的备份，确认完整清单后逐项移到 Windows 回收站；不改变 GPU 设置，不会自动清除暂时无法恢复的备份。
...
6. 在“备份与日志 → 备份管理”选择一条或多条备份，点击“清除所选备份…”并核对文件清单，即可移到回收站。按住 Ctrl 点击可多选，Shift 点击可连续选择，列表获得焦点后 Ctrl+A 可全选；恢复备份仍需单选。恢复资格不代表备份是否有用；旧备份可能在恢复较新备份后重新符合恢复条件。
...
列表中的“文件不存在”表示记录的 EXE 路径目前找不到，常见于程序移动、卸载或盘符变化。用 Ctrl / Shift 多选或在列表内按 Ctrl+A 全选确认不再使用的条目，再点击 **“清除所选路径…”**，统一检查完整路径及修改预览后确认即可。多选时仅支持清除；修改偏好或检查应用需单选。该操作删除所选路径的整条图形设置记录，包括 GPU 偏好、Auto HDR、窗口化优化等参数；**不会删除 EXE 或文件夹**。“恢复自动选择”只移除 GPU 指定，其他参数仍会保留，因此记录可能继续显示。

<img src="docs/images/application-settings.png" alt="应用设置与清除所选路径按钮，路径为演示数据" width="900">
...
**2.1.1 本次未运行自动化测试、界面烟雾测试或实际清除测试。**以下是 2.1.0 的历史验证记录，不代表新增多选功能已通过测试：
...
Qt / PySide6 6.11.2 按 LGPLv3 使用；对应源码压缩包随 [v2.1.1 Release](https://github.com/Freeze7y/gpu-selector/releases/tag/v2.1.1) 提供。可按本仓库构建步骤重新构建程序，并依第三方说明替换或重建库。本程序按现状提供，不提供担保。
...
**2.1.1 is a stable release** adding multi-selection to backup cleanup and application-path removal (Ctrl / Shift / Ctrl+A). Backup restoration and preference editing remain single-selection operations. No tests were run for this release; the verification results above are from 2.1.0. GitHub update checks and in-place EXE updates remain available. Startup checks are opt-in and each available update requires confirmation. Stable installations stay on the stable release channel. Settings, profiles and GPU backups are preserved. Actual GPU configuration switching, UAC, post-reboot switching and execution on Windows 10 remain untested; see the [compatibility notes](docs/WINDOWS10_COMPATIBILITY.md). Probe results apply only to the probe process.
