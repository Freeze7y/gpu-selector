...
- 备份清除与清除所选路径支持 Ctrl / Shift 多选及列表内 Ctrl+A 全选。
- 备份批量清除统一确认清单，逐项移到回收站并显示成功和失败结果；恢复备份仍需单选。
- 多条应用路径合并为一次修改预览，沿用自动备份、读回与失败恢复流程；多选时禁用单应用偏好操作。
- 刷新保留仍存在的所选条目，清除后不自动选中其他条目。
- 本次未运行测试；2.1.0 的历史验证记录不作为本次多选功能的测试结论。
